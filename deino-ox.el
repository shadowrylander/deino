;;; deino-ox.el --- Org mode export widget implemented in deino
...
;;; Commentary:
;;
;; This shows how a complex dispatch menu can be built with deino.
...
(defdeino deino-ox-html (:color blue) nil
  ...
  ("H" (org-html-export-as-html
        ...
        (eq deino-ox/export-scope 'subtree)
        deino-ox/visible-only
        deino-ox/body-only)
       "As HTML buffer")
  ("h" (org-html-export-to-html
        ...
        (eq deino-ox/export-scope 'subtree)
        deino-ox/visible-only
        deino-ox/body-only) "As HTML file")
  ("o" (org-open-file
        (org-html-export-to-html
         deino-ox/async-export
         (eq deino-ox/export-scope 'subtree)
         deino-ox/visible-only
         deino-ox/body-only)) "As HTML file and open")
...
_C-a_ Async export: %`deino-ox/async-export

"
  ("C-b" (deino-ox/body-only) nil)
  ("C-v" (deino-ox/visible-only) nil)
  ("C-s" (deino-ox/export-scope) nil)
  ("C-f" (deino-ox/force-publishing) nil)
  ("C-a" (deino-ox/async-export) nil)
  ("h" deino-ox-html/body "Export to HTML" :exit t)
  ("l" deino-ox-latex/body "Export to LaTeX" :exit t)
  ("t" deino-ox-text/body "Export to Plain Text" :exit t)
  ("q" nil "quit"))

(define-key org-mode-map (kbd "C-c C-,") 'deino-ox/body)

(provide 'deino-ox)

;;; deino-ox.el ends here
